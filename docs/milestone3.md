...
In moving our search algorithm from the MATLAB simulation to C, we found it worthwhile to write our own implementation of Dijkstra's shortest path algorithm because we found none online that fit our already existing data structures particularly well. In writing our own, we were able to closely limit the size of objects to exactly the maximum that may be need within our 5x4 maze, in order to optimize data usage. The final algorithm ended up with a much lower memory consumption than expected, requiring a maximum of only 65 bytes of dynamic memory, excluding the already existing global variables. 
