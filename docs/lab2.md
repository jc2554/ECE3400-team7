...
## Sub- teams:
### Optical 
1. JinJie Chen; Kenneth Huaman; Adrian 
...
In order to convert from time to frequency domain we calculate this sum (the fourier transform):
![](./image/lab2/FREQ.png)

In order to convert from frequency to time domain we calculate this sum (the fourier transform):
![](./image/lab2/TIME.png)
